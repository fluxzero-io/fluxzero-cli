...
- **Dev server startup**: Stable dev-server releases now fall back to the project-local Maven or Gradle resolver when
  the native direct download fails, so first-run startup can use existing proxy, mirror, and dependency-cache settings
  instead of ending with an unhelpful download exception.
- **Dev server startup**: Native CLI installations now use `java` from `PATH` when neither `JAVA_HOME` nor
  `java.home` identifies a Java runtime, instead of failing with a message-less `NullPointerException`.
