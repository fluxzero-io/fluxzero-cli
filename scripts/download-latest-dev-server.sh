...
repository=https://packages.fluxzero.io/maven
metadata_url="$repository/io/fluxzero/tools/fluxzero-dev-server/maven-metadata.xml"
metadata=$(mktemp)
...
if not versions:
    raise SystemExit("Fluxzero Packages metadata contains no stable 1.x Fluxzero dev server")
print(max(versions)[2])
PY
)

mkdir -p "$(dirname "$output")"
artifact_url="$repository/io/fluxzero/tools/fluxzero-dev-server/$version/fluxzero-dev-server-$version-standalone.jar"
