...
internal const val PRIMARY_LATEST_API_URL =
    "https://api.github.com/repos/fluxzero-io/fluxzero-cli/releases/latest"
...
open class DefaultInstallationService(
        ...
        return when {
            current == null -> {
                try {
                    ...
                    throw IllegalStateException(
                        "Installation failed: ${e.message}. Please try reinstalling using the installation script at https://fluxzero.io/docs/getting-started",
                        e
                    )
                }
            }
            ...
            updateService.isNewer(current, latest) -> {
                try {
                    ...
                    throw IllegalStateException(
                        "Upgrade failed: ${e.message}. Please try reinstalling using the installation script at https://fluxzero.io/docs/getting-started",
                        e
                    )
                }
            }
    ...
    private fun fetchLatestTag(): String? {
        return tryFetchLatestTag(PRIMARY_LATEST_API_URL)
    }
    ...
    private fun downloadBinary(tag: String, binaryName: String): HttpResponse<InputStream>? {
        ...
        return tryDownloadBinary(primaryUrl)
    }
